...
def _select_target_kernel_rows(rows: list[dict]) -> list[dict]:
...
def _parse_ncu_csv(csv_text: str) -> list[dict]:
    ...
    target_rows = _select_target_kernel_rows(buffered_rows)
...
def _run_ncu_profile(
    *,
    ...
    # For CUDA / CUTLASS backends we can filter to `solve`. For Triton the
    # kernel names are mangled / autogenerated so we disable the filter.
    if backend != "triton" and not no_kernel_filter:
        cmd += ["-k", "solve"]
...
def _rank_by_axis(agg: dict[str, dict], ncu_num: int) -> dict[str, list]:
    """Return the top-ncu_num metrics per axis, sorted by 'severity'."""
    ...
    for name, info in agg.items():
        ...
        if value is None:
            continue
        # Severity score: if higher is worse, larger is more severe; if
        # lower is worse (e.g. hit rate, occupancy), invert.
        sev = value if higher_is_worse else -value
        by_axis[axis].append({
            "name": name,
            "value": value,
            "unit": info.get("unit"),
    ...
    for axis in by_axis:
        by_axis[axis].sort(key=lambda m: m["severity"], reverse=True)
        by_axis[axis] = by_axis[axis][:ncu_num]
        # strip the scratch field
...
def main() -> None:
    ...
    ncu_bin = args.ncu_bin or state.get("env", {}).get("ncu", {}).get("path") or shutil.which("ncu") or "ncu"
    ...
    if not ncu_info.get("available", True) or shutil.which(ncu_bin) is None:
        top = {
            "degraded": True,
