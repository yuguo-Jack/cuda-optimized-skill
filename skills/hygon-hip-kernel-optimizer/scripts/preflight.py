...
Checks:
  Baseline
    .hip/.cu/.cpp/.cc/.cxx  →  contains `extern "C" void solve(...)` with parseable params
    .py  →  importable, exposes `setup(**kwargs)` AND `run_kernel(**kwargs)`
  Reference
    .py  →  importable, exposes `reference(**kwargs)` (+ optional atol/rtol)
  Dims
    Every int/long/size_t parameter in the HIP signature has a matching
...
# ---------------------------------------------------------------------------
# HIP source inspection (mirrors benchmark.py's parse_solve_signature)
# ---------------------------------------------------------------------------
...
def _parse_solve(source_path: str) -> list[tuple[str, str, bool]]:
    src = Path(source_path).read_text(encoding="utf-8", errors="ignore")
    m = _SIG.search(src)
    if not m:
        raise ValueError(
    ...
    for tok in raw.split(","):
        ...
        for prefix in sorted(
            ...
            if mm:
                ...
                break
        if not matched:
            raise ValueError(f"{source_path}: cannot parse parameter '{tok}' in solve signature")
    return out
...
def _import_without_executing_cuda(path: str, name: str):
    ...
    try:
        ...
        # or a backend dependency fails on a headless CI-style host.
...
def _check_python_backend(py_path: str) -> dict:
    ...
    if missing:
        raise AttributeError(
            f"{py_path}: Python backend module missing required callables: {', '.join(missing)}"
        )
    try:
...
def _check_dims_hip(sig: list[tuple[str, str, bool]], dims: dict) -> list[str]:
    missing = []
...
def run(baseline: str, ref: str, dims: dict, strict_ref_params: bool = False) -> dict:
    ...
    try:
        if ext in {".hip", ".cu", ".cpp", ".cc", ".cxx"}:
            sig = _parse_solve(baseline)
            report["baseline"] = {
                "path": baseline,
                "backend": "hip_or_ck_tile",
                "signature": [
            ...
            report["baseline"] = _check_python_backend(baseline)
            # Python: we can't statically know which dims setup() needs, but
            # we can warn when the dict is empty.
            if not dims:
                report["warnings"].append(
                    "baseline: Python backend module — no dims supplied; setup() will "
            ...
            report["ok"] = False
            report["errors"].append(
                f"baseline: unsupported extension '{ext}' (expected .hip/.cu/.cpp/.cc/.cxx or .py)"
            )
            return report
    except Exception as e:
        report["ok"] = False
    ...
    if strict_ref_params and ext in {".hip", ".cu", ".cpp", ".cc", ".cxx"}:
