...
All DCU validation must run on the remote compute node, not on the login node. The current project workflow has no Docker layer:

```text
local workspace -> login node 10.102.13.101 -> compute node gc02r3n15
```

On the shared filesystem the repository is:

```text
/share_zhipu/home/zptest/yuguo/cuda-optimized-skill
```
...
```bash
cd /share_zhipu/home/zptest/yuguo/cuda-optimized-skill
